...
#+ATTR_REVEAL: :frag appear
For \(\beta^{(1)}_{igs}\), \\
$\beta^{(j)}_{igs} = \beta^{(j)}_0 + \gamma^{(j)}_{gs}$
...
For \(\beta^{(3)}_{igsc}\), \\
\begin{split}
\beta^{(3)}_{ig sc} = \beta^{(3)}_0 + \tau_1 * \text{precip}_c + \tau_2 * \text{gdd}_c + \tau_3 *(\text{precip}_c * \text{gdd}_c) + \\
...
For \(\beta^{(3)}_{igsc}\), \\
\begin{split}
\color{gray}{\beta^{(3)}_{ig sc} = \beta^{(3)}_0 + }  \tau_1 * \text{precip}_c + \tau_2 * \text{gdd}_c + \tau_3 *(\text{precip}_c * \text{gdd}_c) \color{gray} {+ \\
\gamma^{(3)}_{gs}+\delta^{(3)}_{c}}
\end{split}
