...
:END:
#+begin_src R
mod <- readRDS("~/git/allo/models/genus_none_species_many_cities_single_climate_none_hetero_no_family_Gamma.rds")
#+end_src
...
#+begin_src R :exports results :results graphics :file ../figs/marginal_effects_BySpecies_none_species_many_cities_single_climate_none_hetero_no_family_Gamma.png :width 800
  cond <- data.frame(Species = unique(d$Species))
  me <- marginal_effects(mod, conditions = cond, re_formula = NULL)
...
*** genus: many;  species: many; cities: many; climate: b0linint; hetero: no; family: gaussian
:PROPERTIES:
...
#+RESULTS:
: erker@krusty's password:
: building file list ... rsync: link_stat "/Users/erker/git/allo/code/genus_many_species_many_cities_many_climate_b0linint_hetero_no_family_gaussian.R" failed: No such file or directory (2)
: done
:
: sent 29 bytes  received 20 bytes  5.16 bytes/sec
