...
\[
\mu = \beta_0 + \beta_1 (1 - \exp(\beta_2 x^{\beta_3}))
\]
...
for each beta, i, species, j, and each city, k.

\[
\beta_{ijk} \sim Gamma(\mu_i} + \gamma_{ij}+\delta_{ik}, \alpha_{i})
\]

where \mu_i is the mean for \beta coefficient i.
