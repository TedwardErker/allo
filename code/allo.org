...
We fit models of generally increasing complexity and then ..
...
Computed from 8000 by 5381 log-likelihood matrix

         Estimate    SE
elpd_loo -20513.1  81.6
p_loo         4.9   0.2
looic     41026.2 163.1
------
...
Computed from 6000 by 5381 log-likelihood matrix

         Estimate    SE
elpd_loo -19764.5  88.7
p_loo        47.6   3.8
looic     39529.0 177.4
------
...
Pareto k diagnostic values:
                         Count Pct.    Min. n_eff
(-Inf, 0.5]   (good)     5379  100.0%  473
 (0.5, 0.7]   (ok)          2    0.0%  812
   (0.7, 1]   (bad)         0    0.0%  <NA>
...
Computed from 24000 by 5381 log-likelihood matrix

         Estimate    SE
elpd_loo -18989.2  94.1
p_loo       185.8  12.2
looic     37978.5 188.2
------
...
Pareto k diagnostic values:
                         Count Pct.    Min. n_eff
(-Inf, 0.5]   (good)     5367  99.7%   2583
 (0.5, 0.7]   (ok)         11   0.2%   316
   (0.7, 1]   (bad)         2   0.0%   398
...
Computed from 2000 by 5381 log-likelihood matrix

         Estimate    SE
elpd_loo -20195.2  82.6
p_loo         7.0   0.3
looic     40390.4 165.2
------
Monte Carlo SE of elpd_loo is 0.1.
...
Computed from 4000 by 5381 log-likelihood matrix

         Estimate    SE
elpd_loo -20180.4  82.8
p_loo         6.8   0.3
looic     40360.8 165.5
------
Monte Carlo SE of elpd_loo is 0.0.
...
Computed from 36000 by 5381 log-likelihood matrix

         Estimate    SE
elpd_loo -18845.4  95.9
p_loo       163.6  14.5
looic     37690.8 191.7
------
Monte Carlo SE of elpd_loo is NA.
...
Pareto k diagnostic values:
                         Count Pct.    Min. n_eff
(-Inf, 0.5]   (good)     5363  99.7%   1638
 (0.5, 0.7]   (ok)         12   0.2%   801
   (0.7, 1]   (bad)         5   0.1%   201
...
Pareto k diagnostic values:
                         Count Pct.    Min. n_eff
(-Inf, 0.5]   (good)     5370  99.8%   4725
 (0.5, 0.7]   (ok)          7   0.1%   623
   (0.7, 1]   (bad)         4   0.1%   77
   (1, Inf)   (very bad)    0   0.0%   <NA>
See help('pareto-k-diagnostic') for details.
...
model6      0.0  -18845.4     95.9       163.6     14.5  37690.8    191.7
model7   -131.0  -18976.4     93.7       142.4      9.7  37952.8    187.3
model3   -143.8  -18989.2     94.1       185.8     12.2  37978.5    188.2
model2   -919.1  -19764.5     88.7        47.6      3.8  39529.0    177.4
model5  -1335.0  -20180.4     82.8         6.8      0.3  40360.8    165.5
model4  -1349.8  -20195.2     82.6         7.0      0.3  40390.4    165.2
model1  -1667.7  -20513.1     81.6         4.9      0.2  41026.2    163.1
#+end_example
...
Short descriptions of the models tested and the brms sytax are in
table [[tab:model_descriptions]]


Note, I removed the scaling of the parameters (multiplying and
dividing by 100) in the formula for clarity.  In the code they are
scaled so that the parameters are on the same order of magnitude and
HMC sampling is improved.
...
|       |                                              | b2 \sim 1                                               |
|       |                                              | b3 \sim 1                                               |
|     2 | Parameters vary by city                      | DBH ~ b0 + b1 * (1 - exp(-(b2) * AGE^(b3)))           |
|       |                                              | b0 \sim (1 \vert City)                                      |
|       |                                              | b1 \sim (1 \vert City)                                      |
|       |                                              | b2 \sim (1 \vert City)                                      |
|       |                                              | b3 \sim (1 \vert City)                                      |
|     3 | Parameters vary by genus and species         | DBH ~ b0 + b1 * (1 - exp(-(b2) * AGE^(b3)))           |
...
|     4 | Asympotote (\beta_1) varies by climate            | DBH ~ b0 + b1 * (1 - exp(-(b2) * AGE^(b3)))           |
|       |                                              | b0 ~ 1                                               |
|       |                                              | b1 ~ gdd * precip                                    |
|       |                                              | b2 ~ 1                                               |
|       |                                              | b3 ~ 1                                               |
|     5 | Growth rate (\beta_3) varies by climate           | DBH ~ b0 + b1 * (1 - exp(-(b2) * AGE^(b3)))           |
|       |                                              | b0 ~ 1                                               |
|       |                                              | b1 ~ 1                                               |
|       |                                              | b2 ~ 1                                               |
|       |                                              | b3 ~ gdd * precip                                    |
|     6 | Parameters vary by city, genus, and species. | DBH ~ b0 + b1 * (1 - exp(-(b2) * AGE^(b3)))           |
...
|     7 | Parameters vary by city, genus, and species  | DBH ~ b0 + b1 * (1 - exp(-(b2) * AGE^(b3)))          |
|       | (but asympote does not vary by city).        | b0 \sim (1 \vert City) + (1 \vert Genus/Species)                |
