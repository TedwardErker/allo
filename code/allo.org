...
#+RESULTS:
:
: [1] 32 14 20 22  9 11  3  7 10 20 26 26
...
sd(b0_Intercept)                  13.03     17.25     0.28    59.06          1
sd(b1_Intercept)                   1.46      1.53     0.05     5.81          2
sd(b2_Intercept)                   0.16      0.13     0.01     0.51        148
cor(b0_Intercept,b1_Intercept)     0.26      0.49    -0.78     0.94          4
cor(b0_Intercept,b2_Intercept)     0.16      0.52    -0.85     0.95          4
cor(b1_Intercept,b2_Intercept)     0.20      0.51    -0.82     0.96          5
                               Rhat
sd(b0_Intercept)               1.54
sd(b1_Intercept)               1.22
sd(b2_Intercept)               1.03
cor(b0_Intercept,b1_Intercept) 1.09
cor(b0_Intercept,b2_Intercept) 1.12
cor(b1_Intercept,b2_Intercept) 1.08

Population-Level Effects:
             Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
b0_Intercept     1.20      0.52     0.41     2.50        823 1.01
b1_Intercept     1.27      0.63     0.25     2.27          1 1.67
b2_Intercept     0.79      0.21     0.48     1.12          1 2.28

Family Specific Parameters:
      Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sigma     4.54      1.56     2.66     6.83          1 5.07
...
2: There were 229 divergent transitions after warmup. Increasing adapt_delta above 0.8 may help.
...
#+begin_src R :var clim=city_climate
  d <- readRDS("../data/tidy_age_dbh.rds")
  list <- filter(d, SpCode %in% c("LIST","GLTR","ACRU","ACSA1"))
  list <- filter(d, SpCode %in% c("ACRU","ACSA1","ACSA2","ACPL", "ACNE"))
...
#+begin_src R :exports results :results graphics :file ../figs/list_clim.png :height 200 :width 1000
pc <-   ggplot(list_clim, aes(x = AGE, y = DBH, color = City)) + geom_point()
pp <-   ggplot(list_clim, aes(x = AGE, y = DBH, color = Precip)) + geom_point()
ph <-   ggplot(list_clim, aes(x = AGE, y = DBH, color = HDD)) + geom_point()
...
#+begin_src R :exports results :results graphics :file ../figs/list_clim_sepcities.png
  pc <-   ggplot(list_clim, aes(x = AGE, y = DBH, color = CDD, shape = SpCode)) + geom_point() +
...
#+begin_src R
list_clim <- select(list_clim, DBH, AGE, City, Precip, HDD)
#+end_src
...
#+begin_src R
  ...
  model_list_climate <- brm(form, chains = 2, cores = 2, data = list_clim, prior = nlprior, init_r = .2)
...
Formula: DBH ~ 100 * b0 * (1 - exp(-(b1/100) * AGE^b2))
         ...
         b1 ~ Precip * HDD + (1 | City)
         b2 ~ Precip * HDD + (1 | City)
   Data: list_clim (Number of observations: 428)
Samples: 2 chains, each with iter = 2000; warmup = 1000; thin = 1;
         total post-warmup samples = 2000

Group-Level Effects:
~City (Number of levels: 11)
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sd(b0_Intercept)     0.96      0.70     0.02     2.47         42 1.03
sd(b1_Intercept)     2.60      0.97     1.20     5.00        255 1.02
sd(b2_Intercept)     0.13      0.11     0.00     0.43         88 1.02

Population-Level Effects:
              Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
b0_Intercept      1.38      0.38     0.67     2.09         77 1.01
b0_Precip         0.74      0.36     0.23     1.65        614 1.00
b0_HDD            0.99      0.41     0.31     1.85         93 1.01
b0_Precip:HDD     1.79      0.73     0.57     3.30         70 1.01
b1_Intercept      2.16      0.75     0.99     3.96        310 1.00
b1_Precip         2.51      1.13     0.82     5.17        514 1.00
b1_HDD            2.27      0.90     0.89     4.46        406 1.00
b1_Precip:HDD     4.20      1.58     1.71     7.75        652 1.00
b2_Intercept      1.00      0.07     0.86     1.15        376 1.00
b2_Precip         0.35      0.14     0.14     0.67        260 1.00
b2_HDD            0.23      0.07     0.11     0.39        300 1.00
b2_Precip:HDD     0.46      0.16     0.22     0.84        333 1.00

Family Specific Parameters:
      Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sigma     7.91      0.28     7.40     8.46       1290 1.00

Samples were drawn using sampling(NUTS). For each parameter, Eff.Sample
is a crude measure of effective sample size, and Rhat is the potential
scale reduction factor on split chains (at convergence, Rhat = 1).
Warning message:
There were 48 divergent transitions after warmup. Increasing adapt_delta above 0.8 may help.
See http://mc-stan.org/misc/warnings.html#divergent-transitions-after-warmup
#+end_example

**** diagnostics
#+begin_src R :exports results :results graphics :file ../figs/mcmctrace_fram_climate_homo.png :height 1000 :width 1000
mcmc_trace(as.matrix(model_list_climate))
#+end_src
...
**** marginal effects
#+begin_src R :exports results :results graphics :file ../figs/marginal_effects_age_list_age_homo.png
marginal_effects(model_list_climate, effects = "AGE:Precip")
#+end_src

#+RESULTS:
[[file:../figs/marginal_effects_age_list_age_homo.png]]

#+begin_src R :exports results :results graphics :file ../figs/marginal_effects_age_list_age_HDD_homo.png
marginal_effects(model_list_climate, effects = "AGE:HDD")
#+end_src

#+RESULTS:
[[file:../figs/marginal_effects_age_list_age_HDD_homo.png]]


#+begin_src R :exports results :results graphics :file ../figs/marginal_effects_age_list_surface_homo.png
marginal_effects(model_list_climate, effects = "Precip:HDD", surface = T)
#+end_src

#+RESULTS:
[[file:../figs/marginal_effects_age_list_surface_homo.png]]


#+begin_src R :exports results :results graphics :file ../figs/marginal_effects_age_list_climate_homo_Points.png
    conditions <- unique(select(list_clim, Precip, HDD))

  me <- marginal_effects(model_list_climate, effects = "AGE", conditions = conditions, re_formula = NA)

    ggplot(me$AGE) +
          geom_line(aes(x = AGE, y = estimate__)) +
        geom_ribbon(aes(x = AGE, ymin = estimate__ - se__, ymax = estimate__ + se__), alpha = .3, fill = "blue") +
        geom_point(data = list_clim, aes(x = AGE, y = DBH)) +
        facet_wrap(~Precip)
#+end_src

#+RESULTS:
[[file:../figs/marginal_effects_age_list_climate_homo_Points.png]]
terrible fit.
...
Very cool, there is an interaction between Precip and HDD.

THe behavior isn't what I had expected, but I need more than just 4 cities!
...
#+begin_src R
  ...
  #  model_multipleSpecies_nestedGenus_multipleCities_homo <- update(m_generated_multipleSpecies_nestedGenus_multipleLocations_homo, newdata = dm, cores = 2, chains = 2)
  #, control = list(adapt_delta = .9, max_treedepth = 15))
    ...
    nlprior <- c(prior(gamma(4, 3), nlpar = "b0", lb = 0),
                 prior(gamma(5, 1.2), nlpar = "b1", lb = 0),
                 prior(gamma(4, 3), nlpar = "b2", lb = 0),
                 prior(cauchy(0,.3), class = "sd", nlpar = "b0"),
                 prior(cauchy(0,.2), class = "sd", nlpar = "b1"),
                 prior(cauchy(0,.2), class = "sd", nlpar = "b2"))
...
#  get_prior(form, dm)
    ...
    model_multipleSpecies_nestedGenus_multipleCities_homo <- brm(form, chains = 2, cores = 2, data = dm, prior = nlprior, init_r = .4)
...
Compiling the C++ model
Start sampling
Error : 'control' list contains unknown members of names: init_r.
...
#+BEGIN_SRC stan
  ...
  transformed parameters {
    ...
    // group-level effects
    vector[N_6] r_6_b1_1 = sd_6[1] * (z_6[1]);
    // group-level effects
    vector[N_7] r_7_b2_1 = sd_7[1] * (z_7[1]);
    // group-level effects
    vector[N_8] r_8_b2_1 = sd_8[1] * (z_8[1]);
    // group-level effects
    vector[N_9] r_9_b2_1 = sd_9[1] * (z_9[1]);
  }
  model {
    vector[N] mu_b0 = X_b0 * b_b0;
    vector[N] mu_b1 = X_b1 * b_b1;
    ...
    vector[N] mu;
    for (n in 1:N) {
      mu_b0[n] += r_1_b0_1[J_1[n]] * Z_1_b0_1[n] + r_2_b0_1[J_2[n]] * Z_2_b0_1[n] + r_3_b0_1[J_3[n]] * Z_3_b0_1[n];
      mu_b1[n] += r_4_b1_1[J_4[n]] * Z_4_b1_1[n] + r_5_b1_1[J_5[n]] * Z_5_b1_1[n] + r_6_b1_1[J_6[n]] * Z_6_b1_1[n];
      mu_b2[n] += r_7_b2_1[J_7[n]] * Z_7_b2_1[n] + r_8_b2_1[J_8[n]] * Z_8_b2_1[n] + r_9_b2_1[J_9[n]] * Z_9_b2_1[n];
      // compute non-linear predictor
      mu[n] = 100 * mu_b0[n] * (1 - exp( - (mu_b1[n] / 100) * C_1[n] ^ mu_b2[n]));
    }
    // priors including all constants
    target += gamma_lpdf(b_b0 | 4, 3)
      - 1 * gamma_lccdf(0 | 4, 3);
    target += gamma_lpdf(b_b1 | 5, 1.2)
      - 1 * gamma_lccdf(0 | 5, 1.2);
    target += gamma_lpdf(b_b2 | 4, 3)
      - 1 * gamma_lccdf(0 | 4, 3);
    target += student_t_lpdf(sigma | 3, 0, 33)
      - 1 * student_t_lccdf(0 | 3, 0, 33);
    target += cauchy_lpdf(sd_1 | 0, 0.3)
      - 1 * cauchy_lccdf(0 | 0, 0.3);
    target += normal_lpdf(z_1[1] | 0, 1);
    target += cauchy_lpdf(sd_2 | 0, 0.3)
      - 1 * cauchy_lccdf(0 | 0, 0.3);
    target += normal_lpdf(z_2[1] | 0, 1);
    target += cauchy_lpdf(sd_3 | 0, 0.3)
      - 1 * cauchy_lccdf(0 | 0, 0.3);
    target += normal_lpdf(z_3[1] | 0, 1);
    target += cauchy_lpdf(sd_4 | 0, 0.2)
      - 1 * cauchy_lccdf(0 | 0, 0.2);
    target += normal_lpdf(z_4[1] | 0, 1);
    target += cauchy_lpdf(sd_5 | 0, 0.2)
      - 1 * cauchy_lccdf(0 | 0, 0.2);
    target += normal_lpdf(z_5[1] | 0, 1);
    target += cauchy_lpdf(sd_6 | 0, 0.2)
      - 1 * cauchy_lccdf(0 | 0, 0.2);
    target += normal_lpdf(z_6[1] | 0, 1);
    target += cauchy_lpdf(sd_7 | 0, 0.2)
      - 1 * cauchy_lccdf(0 | 0, 0.2);
    target += normal_lpdf(z_7[1] | 0, 1);
    target += cauchy_lpdf(sd_8 | 0, 0.2)
      - 1 * cauchy_lccdf(0 | 0, 0.2);
    target += normal_lpdf(z_8[1] | 0, 1);
    target += cauchy_lpdf(sd_9 | 0, 0.2)
      - 1 * cauchy_lccdf(0 | 0, 0.2);
...
#+begin_src R
  model_multipleSpecies_nestedGenus_multipleCities_homo
#+end_src
...
~City (Number of levels: 4)
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sd(b0_Intercept)     0.59      0.34     0.21     1.41        220 1.02
sd(b1_Intercept)     0.31      0.18     0.02     0.73         20 1.07
sd(b2_Intercept)     0.24      0.12     0.07     0.57         67 1.02

~Genus (Number of levels: 3)
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sd(b0_Intercept)     0.20      0.24     0.00     0.84         80 1.03
sd(b1_Intercept)     0.22      0.23     0.01     0.85         18 1.08
sd(b2_Intercept)     0.11      0.09     0.01     0.38         60 1.03

~Genus:Species (Number of levels: 8)
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sd(b0_Intercept)     0.51      0.20     0.25     1.02        202 1.00
sd(b1_Intercept)     0.30      0.13     0.01     0.59         35 1.05
sd(b2_Intercept)     0.09      0.04     0.03     0.17        228 1.03

Population-Level Effects:
             Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
b0_Intercept     1.45      0.38     0.72     2.20        169 1.00
b1_Intercept     1.04      0.24     0.55     1.47        109 1.01
b2_Intercept     1.08      0.15     0.80     1.39         73 1.01

Family Specific Parameters:
      Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sigma     8.20      0.19     7.85     8.60         53 1.04

Samples were drawn using sampling(NUTS). For each parameter, Eff.Sample
is a crude measure of effective sample size, and Rhat is the potential
scale reduction factor on split chains (at convergence, Rhat = 1).
Warning message:
There were 20 divergent transitions after warmup. Increasing adapt_delta above 0.8 may help.
See http://mc-stan.org/misc/warnings.html#divergent-transitions-after-warmup
#+end_example
...
pairs(model_multipleSpecies_nestedGenus_multipleCities_homo)
#+end_src
...
#+begin_src R :exports results :results graphics :file ../figs/marginal_model_multipleSpecies_nestedGenus_multipleCities_homo.png
     cond <- unique(select(dm, City, Species, Genus))
    me <- marginal_effects(model_multipleSpecies_nestedGenus_multipleCities_homo,
                       re_formula = NULL, method = "predict",
                          conditions = cond)
...
#+RESULTS:
[[file:../figs/marginal_model_multipleSpecies_nestedGenus_multipleCities_homo.png]]

#+begin_src R :exports results :results graphics :file ../figs/marginal2_model_multipleSpecies_nestedGenus_multipleCities_homo.png
  City = unique(dm$City)
...
#+RESULTS:
[[file:../figs/marginal2_model_multipleSpecies_nestedGenus_multipleCities_homo.png]]
