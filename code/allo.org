...
#+begin_src R :var city_climate=city_climate
  ...
  cities <- city_climate %>%
    ...
    d <- bind_cols(cdd@data,cities@data, data.frame(coordinates(cities))) %>%
        select(cdd, Precip, City, Longitude, Latitude) %>%
      rename(GDD = cdd)
...
#+begin_src R :exports results :results graphics :file ../figs/CDD_Precip_Cities.png
  ggplot(d, aes(x = CDD, y = Precip, label = City)) + geom_text()
#+end_src

#+RESULTS:
[[file:../figs/CDD_Precip_Cities.png]]
