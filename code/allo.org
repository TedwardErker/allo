...
#+begin_src R
  d <- d %>% rename(DBH = `DBH (cm)`, Leaf = `Leaf (m2)`, Species = SpCode) %>% select(Region, City, TreeID, SpCode, DBH, Leaf, DBH, Age, ScientificName)
#+end_src

#+RESULTS:
: Error in FUN(X[[i]], ...) : object 'SpCode' not found
