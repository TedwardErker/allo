...
#+BEGIN_SRC sh
wget -O ../data/cdd50.txt https://www1.ncdc.noaa.gov/pub/data/normals/1981-2010/products/temperature/ann-cldd-base50.txt
...
#+begin_src R
  ...
  write.csv(dl, "../data/cdd50_qt_ll.csv")
#+end_src
...
#+begin_src R :var city_climate=city_climate
  ...
  cities <- city_climate %>%
      ...
      cdd <- read.csv("../data/cdd50_qt_ll.csv", stringsAsFactors =F)
    ...
    d <- bind_cols(cdd@data,cities@data, data.frame(coordinates(cities))) %>%
        select(cdd, city, longitude, latitude, population)

    write.csv(d, "../data/cities_cdd50.csv")
#+end_src


#+begin_src R :var city_climate=city_climate
    cdd <- read.csv("../data/cities_cdd50.csv", stringsAsFactors = F)

  str(city_climate)
    test <- left_join(city_climate,cdd)
  str(test)
...
'data.frame':	17 obs. of  6 variables:
 ...
 $ city     : chr  "Orlando" "Charleston" "Claremont" "Modesto" ...
 $ CityState: chr  "Orlando, FL" "Charleston, SC" "Claremont, CA" "Modesto, CA" ...
...
'data.frame':	19 obs. of  11 variables:
 $ Region    : chr  "CenFla" "GulfCo" "GulfCo" "InlEmp" ...
 $ city      : chr  "Orlando" "Charleston" "Charleston" "Claremont" ...
 $ CityState : chr  "Orlando, FL" "Charleston, SC" "Charleston, SC" "Claremont, CA" ...
 $ CDD       : int  1806 1124 1124 134 1052 773 677 510 355 349 ...
 $ HDD       : int  289 1221 1221 872 1439 1718 2416 3153 4436 3332 ...
 $ Precip    : int  1367 1555 1555 523 315 470 250 392 622 452 ...
 $ X         : int  77 200 725 NA 105 35 32 12 46 160 ...
 $ cdd       : int  8539 6375 3836 NA 5259 4917 4065 3330 3140 2655 ...
 $ longitude : num  -81.4 -79.9 -81.6 NA -121 ...
 $ latitude  : num  28.5 32.8 38.3 NA 37.6 ...
 $ population: int  255483 127999 50821 NA 204933 479686 556495 843393 400070 152061 ...
#+end_example
...
#+begin_src R :var clim=city_climate
  ...
  #  species.to.filter <- c("LIST","GLTR","ACRU","MAGR","PYCA", "ACSA1", "ACSA2", "ACNE", "FRPE", "FRAM", "FRVE","ACPL", "ULAM", "ULPU", "TIAM", "TICO")
...
sent 84630 bytes  received 42 bytes  33868.80 bytes/sec
total size is 87999  speedup is 1.04
You have new mail in /var/mail/erker
erker@krusty's password:
building file list ... done
age_dbh_testing.rds

sent 132 bytes  received 366 bytes  199.20 bytes/sec
total size is 37472  speedup is 75.24
#+end_example
...
| none  | single  | many   | b0       | no      | DBH ~ 100*b0 * (1 - exp(-(b1/100) * AGE ^ b2)) | b0 ~ t2(Precip, CDD) +  (1 \vert City)                             | b1 ~ (1 \vert City)                             | b2 ~ (1 \vert City)                             |                       |
| none  | single  | many   | b0       | no      | DBH ~ 100*b0 * (1 - exp(-(b1/100) * AGE ^ b2)) | b0 ~ t2(Precip, CDD, k = ) +  (1 \vert City)                       | b1 ~ (1 \vert City)                             | b2 ~ (1 \vert City)                             |                       |
| none  | single  | many   | none     | yes     | DBH ~ 100*b0 * (1 - exp(-(b1/100) * AGE ^ b2)) | b0 ~ (1 \vert City)                                                | b1 ~ (1 \vert City)                             | b2 ~ (1 \vert City)                             | sigma ~ AGE -1        |
| none  | single  | many   | b0       | yes     | DBH ~ 100*b0 * (1 - exp(-(b1/100) * AGE ^ b2)) | b0 ~ t2(Precip, CDD) +  (1 \vert City)                             | b1 ~ (1 \vert City)                             | b2 ~ (1 \vert City)                             | sigma ~ AGE -1        |
...
| many  | many    | many   | b0       | no      | DBH ~ 100*b0 * (1 - exp(-(b1/100) * AGE ^ b2)) | b0 ~ t2(Precip, CDD) +  (1 \vert City) + (1 \vert Genus / Species) | b1 ~ (1 \vert City) + (1 \vert Genus / Species) | b2 ~ (1 \vert City) + (1 \vert Genus / Species) |                       |
| many  | many    | many   | b0linint | no      | DBH ~ 100*b0 * (1 - exp(-(b1/100) * AGE ^ b2)) | b0 ~ Precip * CDD +  (1 \vert City) + (1 \vert Genus / Species)    | b1 ~ (1 \vert City) + (1 \vert Genus / Species) | b2 ~ (1 \vert City) + (1 \vert Genus / Species) |                       |
| many  | many    | many   | b0       | smooth  | DBH ~ 100*b0 * (1 - exp(-(b1/100) * AGE ^ b2)) | b0 ~ t2(Precip, CDD) +  (1 \vert City) + (1 \vert Genus / Species) | b1 ~ (1 \vert City) + (1 \vert Genus / Species) | b2 ~ (1 \vert City) + (1 \vert Genus / Species) | sigma ~ s(AGE)        |
| many  | many    | many   | b0linint | smooth  | DBH ~ 100*b0 * (1 - exp(-(b1/100) * AGE ^ b2)) | b0 ~ Precip * CDD +  (1 \vert City) + (1 \vert Genus / Species)    | b1 ~ (1 \vert City) + (1 \vert Genus / Species) | b2 ~ (1 \vert City) + (1 \vert Genus / Species) | sigma ~ s(AGE)        |
...
| model_table.csv |           |       |          |         |       |       |           |
|                 |           |       |          |         |       |       |           |
| sent            | 208       | bytes | received | 72      | bytes | 112.0 | bytes/sec |
| total           | size      | is    | 3455     | speedup | is    | 12.34 |           |
...
: allo_functions.R
:
: sent 299 bytes  received 60 bytes  4.06 bytes/sec
: total size is 1785  speedup is 4.97
: You have new mail in /var/mail/erker
...
#+begin_src R :exports results :results graphics :file ../figs/marginal_effects_BySpeciesCity_none_species_many_cities_single_climate_none_hetero_no.png :width 1000 :height 1000
  cond <- expand.grid(Species = unique(mod$data$Species), City = unique(mod$data$City))
#  cond <- unique(select(mod$data, Species, City))
  me <- marginal_effects(mod, conditions = cond, re_formula = NULL)
...
*** TODO genus: many;  species: many; cities: many; climate: b0; hetero: no
:PROPERTIES:
...
:
: sent 54470 bytes  received 62280669 bytes  6561593.58 bytes/sec
: total size is 62409970  speedup is 1.00
: You have new mail in /var/mail/erker
...
Samples: 6 chains, each with iter = 5000; warmup = 2500; thin = 1;
         total post-warmup samples = 15000

Smooth Terms:
                      Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sds(b0_t2PrecipHDD_1)     0.20      0.15     0.01     0.56      15000 1.00
sds(b0_t2PrecipHDD_2)     0.20      0.15     0.01     0.56      15000 1.00
sds(b0_t2PrecipHDD_3)     0.20      0.16     0.01     0.58      15000 1.00

Group-Level Effects:
~City (Number of levels: 15)
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sd(b0_Intercept)     0.76      0.35     0.32     1.67       2173 1.00
sd(b1_Intercept)     0.33      0.08     0.21     0.53       7051 1.00
sd(b2_Intercept)     0.13      0.03     0.09     0.20       6350 1.00

~Genus (Number of levels: 18)
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sd(b0_Intercept)     0.28      0.38     0.00     1.36        652 1.01
sd(b1_Intercept)     0.07      0.06     0.00     0.24       2859 1.00
sd(b2_Intercept)     0.05      0.04     0.00     0.14       2672 1.00

~Genus:Species (Number of levels: 25)
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sd(b0_Intercept)     1.02      0.30     0.42     1.65       1316 1.01
sd(b1_Intercept)     0.29      0.06     0.17     0.43       3947 1.00
sd(b2_Intercept)     0.13      0.03     0.08     0.19       3388 1.00

Population-Level Effects:
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
b0_Intercept         2.08      0.34     1.38     2.73       4387 1.00
b0_t2PrecipHDD_1     0.64      0.20     0.33     1.10       5948 1.00
b0_t2PrecipHDD_2     0.45      0.20     0.17     0.93       3256 1.00
b0_t2PrecipHDD_3     0.62      0.21     0.29     1.10       5018 1.00
b1_Intercept         1.10      0.12     0.86     1.35       3411 1.00
b2_Intercept         0.88      0.05     0.78     0.98       3931 1.00

Family Specific Parameters:
      Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sigma     9.71      0.09     9.53     9.90      15000 1.00

Samples were drawn using sampling(NUTS). For each parameter, Eff.Sample
is a crude measure of effective sample size, and Rhat is the potential
scale reduction factor on split chains (at convergence, Rhat = 1).
Warning message:
There were 224 divergent transitions after warmup. Increasing adapt_delta above 0.8 may help.
See http://mc-stan.org/misc/warnings.html#divergent-transitions-after-warmup
#+end_example
...
#+begin_src R :exports results :results graphics :file ../figs/marginal_effects_AGEHDD_genus_many_species_many_cities_many_climate_b0_hetero_no.png
  marginal_effects(mod, effects = "AGE:HDD")
#+end_src

#+RESULTS:
[[file:../figs/marginal_effects_AGEHDD_genus_many_species_many_cities_many_climate_b0_hetero_no.png]]
...
#+begin_src R :exports results :results graphics :file ../figs/marginal_effects_surface_PrecipHDD_genus_many_species_many_cities_many_climate_b0_hetero_no.png
  marginal_effects(mod, effects = "Precip:HDD", surface = T)
#+end_src

#+RESULTS:
[[file:../figs/marginal_effects_surface_PrecipHDD_genus_many_species_many_cities_many_climate_b0_hetero_no.png]]
...
*** TODO genus: many;  species: many; cities: many; climate: b0linint; hetero: no
:PROPERTIES:
...
:
: sent 658 bytes  received 42 bytes  280.00 bytes/sec
: total size is 1141  speedup is 1.63
: You have new mail in /var/mail/erker
...
Formula: DBH ~ 100 * b0linint * (1 - exp(-(b1/100) * AGE^b2))
         b0linint ~ t2(Precip, HDD) + (1 | City) + (1 | Genus/Species)
         b1 ~ (1 | City) + (1 | Genus/Species)
         b2 ~ (1 | City) + (1 | Genus/Species)
   Data: d (Number of observations: 5548)
Samples: 6 chains, each with iter = 5000; warmup = 2500; thin = 1;
...
Smooth Terms:
                      Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sds(b0linint_t2PrecipHDD_1)     0.20      0.15     0.01     0.56      15000 1.00
sds(b0linint_t2PrecipHDD_2)     0.20      0.15     0.01     0.56      15000 1.00
sds(b0linint_t2PrecipHDD_3)     0.20      0.16     0.01     0.58      15000 1.00

Group-Level Effects:
~City (Number of levels: 15)
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sd(b0linint_Intercept)     0.76      0.35     0.32     1.67       2173 1.00
sd(b1_Intercept)     0.33      0.08     0.21     0.53       7051 1.00
sd(b2_Intercept)     0.13      0.03     0.09     0.20       6350 1.00

~Genus (Number of levels: 18)
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sd(b0linint_Intercept)     0.28      0.38     0.00     1.36        652 1.01
sd(b1_Intercept)     0.07      0.06     0.00     0.24       2859 1.00
sd(b2_Intercept)     0.05      0.04     0.00     0.14       2672 1.00

~Genus:Species (Number of levels: 25)
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sd(b0linint_Intercept)     1.02      0.30     0.42     1.65       1316 1.01
sd(b1_Intercept)     0.29      0.06     0.17     0.43       3947 1.00
sd(b2_Intercept)     0.13      0.03     0.08     0.19       3388 1.00

Population-Level Effects:
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
b0linint_Intercept         2.08      0.34     1.38     2.73       4387 1.00
b0linint_t2PrecipHDD_1     0.64      0.20     0.33     1.10       5948 1.00
b0linint_t2PrecipHDD_2     0.45      0.20     0.17     0.93       3256 1.00
b0linint_t2PrecipHDD_3     0.62      0.21     0.29     1.10       5018 1.00
b1_Intercept         1.10      0.12     0.86     1.35       3411 1.00
b2_Intercept         0.88      0.05     0.78     0.98       3931 1.00
...
#+begin_src R :exports results :results graphics :file ../figs/marginal_effects_AGEHDD_genus_many_species_many_cities_many_climate_b0linint_hetero_no.png
  marginal_effects(mod, effects = "AGE:HDD")
#+end_src

#+RESULTS:
[[file:../figs/marginal_effects_AGEHDD_genus_many_species_many_cities_many_climate_b0linint_hetero_no.png]]
...
#+begin_src R :exports results :results graphics :file ../figs/marginal_effects_surface_PrecipHDD_genus_many_species_many_cities_many_climate_b0linint_hetero_no.png
  marginal_effects(mod, effects = "Precip:HDD", surface = T)
#+end_src

#+RESULTS:
[[file:../figs/marginal_effects_surface_PrecipHDD_genus_many_species_many_cities_many_climate_b0linint_hetero_no.png]]
...
*** TODO FULL DATA genus: many;  species: many; cities: many; climate: b0; hetero: smooth
:PROPERTIES:
...
*** genus: many;  species: many; cities: many; climate: b0linint; hetero: smooth
:PROPERTIES:
...
: receiving file list ... done
:
: sent 16 bytes  received 149 bytes  47.14 bytes/sec
: total size is 79316075  speedup is 480703.48
...
Smooth Terms:
                  Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sds(sigma_sAGE_1)     1.32      0.21     0.93     1.76      30000 1.00

Group-Level Effects:
~City (Number of levels: 15)
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sd(b0_Intercept)     0.58      0.20     0.29     1.06         30 1.05
sd(b1_Intercept)     0.32      0.10     0.18     0.56       8209 1.00
sd(b2_Intercept)     0.13      0.03     0.08     0.21      15483 1.00

~Genus (Number of levels: 8)
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sd(b0_Intercept)     0.14      0.14     0.00     0.52       3307 1.01
sd(b1_Intercept)     0.09      0.08     0.00     0.29       9046 1.00
sd(b2_Intercept)     0.04      0.03     0.00     0.12         69 1.02

~Genus:Species (Number of levels: 16)
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sd(b0_Intercept)     0.41      0.12     0.22     0.70         74 1.02
sd(b1_Intercept)     0.20      0.07     0.10     0.36       9499 1.01
sd(b2_Intercept)     0.07      0.03     0.03     0.13          7 1.30

Population-Level Effects:
                Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sigma_Intercept     2.12      0.01     2.09     2.14      30000 1.00
b0_Intercept        1.93      0.35     1.31     2.64          6 1.37
b0_Precip           0.81      0.42     0.20     1.75          6 1.43
b0_HDD              0.47      0.20     0.14     0.86          6 1.39
b0_Precip:HDD       0.84      0.34     0.30     1.62         81 1.02
b1_Intercept        1.12      0.14     0.85     1.42         17 1.09
b2_Intercept        1.00      0.05     0.91     1.11         37 1.04
sigma_sAGE_1        1.10      0.17     0.76     1.44      25237 1.00
...
1: The model has not converged (some Rhats are > 1.1). Do not analyse the results!
We recommend running more iterations and/or setting stronger priors.
2: There were 63 divergent transitions after warmup. Increasing adapt_delta above 0.8 may help.
See http://mc-stan.org/misc/warnings.html#divergent-transitions-after-warmup
 Family: gaussian
  Links: mu = identity; sigma = log
Formula: DBH ~ 100 * b0 * (1 - exp(-(b1/100) * AGE^b2))
...
*** TODO FULL DATA genus: many;  species: many; cities: many; climate: b0linint; hetero: smooth
:PROPERTIES:
...
:
: sent 505 bytes  received 54 bytes  223.60 bytes/sec
: total size is 1142  speedup is 2.04
