...
#+begin_src R :exports results :results graphics :file ../figs/fram_ftcollins_age_dbh.png
ggplot(fram_FtCollins, aes(x = AGE, y = DBH)) + geom_point()
#+end_src

#+RESULTS:
[[file:../figs/fram_ftcollins_age_dbh.png]]

#+begin_src R
          form <- bf(DBH ~ b0 * (1 - exp(-b1 * AGE ^ b2)),
                     b0 ~ 1,
                     b1 ~ 1,
                     b2 ~ 1,
                     nl = T)
