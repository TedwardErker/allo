...
p <- read.table("../data/precip.txt")
  colnames(p) <- c("station", "precip")

  p <- p %>%
      mutate(precip = as.numeric(str_match(precip, "-*[0-9]+")[,1]),
             qual = str_match(precip, "[A-Z]")[,1])
  head(p)
  dim(p)
...
*** TODO genus: many;  species: many; cities: many; climate: b3linint; hetero: no; family: Gamma
:PROPERTIES:
