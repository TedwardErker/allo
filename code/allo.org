...
~City (Number of levels: 4)
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sd(b0_Intercept)     0.58      0.63     0.21     2.36          2 1.32
sd(b1_Intercept)     1.45      0.99     0.20     2.41          1 4.80
sd(b2_Intercept)     0.43      0.21     0.15     0.88        183 1.03

~Genus (Number of levels: 3)
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sd(b0_Intercept)     0.50      0.69     0.05     2.49          5 1.09
sd(b1_Intercept)     1.20      0.89     0.03     2.00          1 2.65
sd(b2_Intercept)     0.22      0.17     0.01     0.72        305 1.01

~Genus:Species (Number of levels: 8)
                 Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sd(b0_Intercept)     0.44      0.37     0.17     1.30          1 1.62
sd(b1_Intercept)     0.67      0.30     0.21     0.95          1 3.05
sd(b2_Intercept)     0.13      0.05     0.04     0.19          2 1.46

Population-Level Effects:
             Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
b0_Intercept     1.63      0.39     0.70     2.24          3 1.20
b1_Intercept     0.80      0.34     0.56     1.65          1 1.55
b2_Intercept     0.93      0.19     0.69     1.40          2 1.38

Family Specific Parameters:
      Estimate Est.Error l-95% CI u-95% CI Eff.Sample  Rhat
sigma     4.40      3.76     0.64     8.42          1 38.18
...
#+begin_src R :exports results :results graphics :file ../figs/marginal_model_multipleSpecies_nestedGenus_multipleCities_homo.png
  ...
  ggplot() +
    geom_line(data = me$AGE, aes(color = Species, x = AGE, y = estimate__)) +
    geom_ribbon(data = me$AGE, alpha = .3, aes(group = Species, x = AGE, ymin = estimate__ - se__, ymax = estimate__ + se__)) +
    geom_point(data = dm, aes(x = AGE, y = DBH)) +
    facet_grid(City ~ Genus)
