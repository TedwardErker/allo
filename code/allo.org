...
[[https://stat.ethz.ch/R-manual/R-devel/library/datasets/html/Orange.html][cite]].  Multilevel modeling .... (why it is effective for this type of
data).  This paper has two main contributions.  First, we demonstrate
the use of Stan via the "brms" package in R to fit bayesian nonlinear
multilevel models.  Second, we apply the method to the Urban Tree
Database cite:utd_2016.  This dataset represents an enormous effort to
collect age and size data on thousands of trees in 17? cities across
the US.
