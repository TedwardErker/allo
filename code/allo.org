...
#+RESULTS:
: Error in bf(data_form, b0_form, b1_form, b2_form, v_form, nl = T) :
:   object 'data_form' not found
...
#+RESULTS:
: Error: Priors on population-level effects are required in non-linear models, but none were found for parameter 'shape'. See help(set_prior) for more details.
: Error in saveRDS(mod, paste0("../models/genus_", genus, "_species_", species,  :
