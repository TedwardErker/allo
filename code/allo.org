...
#+begin_src R
  f <- function(x) {round(255 * (x - min(x, na.rm = T)) / max(x, na.rm = T), 0) }

    d <- left_join(dl, pl) %>%
      select(station, gdd, lat, long, precip) %>%
      filter(complete.cases(.)) %>%
        filter( lat < 50, lat > 25, long < 0) %>%
        mutate(precip_255 = f(sqrt(precip)),
             gdd_255 = f(sqrt(gdd)),
             col = rgb(10,gdd_255, precip_255, maxColorValue = 255))
...
#+begin_src R :exports results :results graphics :file ../figs/space.png
    pal <- expand.grid(gdd = seq(1250,8750,100), precip = seq(100,7000,100)) %>%
      mutate(col = rgb(80, f(gdd), f(precip), maxColorValue = 255))

      ggplot(d, aes(x = gdd, y = precip)) +
...
#+begin_src R :exports results :results graphics :file ../figs/palette.png
    pal <- expand.grid(gdd_255 = 1:255, precip_255 = 1:255) %>%
      mutate(col = rgb(10,gdd_255, precip_255, maxColorValue = 255))

  ggplot(pal, aes(x = gdd_255, y = precip_255, fill = col)) + geom_raster() +
    scale_fill_identity() +
    geom_point(data = dplyr::filter(d, lat < 50, lat > 25, long < 0), aes(x = gdd_255, y = precip_255))
#+end_src

#+RESULTS:
[[file:../figs/palette.png]]
