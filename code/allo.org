...
#+begin_src R :exports results :results graphics :file ../figs/pp_check_bmod.png
pp_check(mod)
#+end_src
...
#+RESULTS:
: Error: Type 'xyz' is not a valid ppc type. Valid types are:
...
*** COMMENT genus: many;  species: many; cities: many (not \beta_1); climate: b3linint; hetero: no; family: Gamma FULL DATA no Palm no multistem species
**** model R code
