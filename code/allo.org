...
fit bayesian nonlinear multilevel models.  Second, we apply the method
to the Urban Tree Database cite:utd_2016.  This dataset is the result
an enormous effort to collect age and size data on thousands of trees
in 17? cities across the US.  Multilevel modelling has the potential
to extract more information from the data and improve predictions
compared to the existing modelling approach for the data.  Improving
...
The urban tree database consists of measurements on XXX trees of XX
species in XX cities.  However, because of the difficulty is measuring
tree age, there are only XX trees with complete age and diameter
data (XX species, XX cities, 309 species x city combinations).
...
\[
\mu_i = \beta_0 + \beta_1 (1 - \exp(\beta_2 x_i^{\beta_3}))
\]
...
for each beta, j, species, k, and each city, l.

\[
\beta_{jkl} \sim Gamma(\mu_j} + \gamma_{jk}+\delta_{jl}, \alpha_{j})
\]

where \mu_j is the mean for \beta coefficient \beta_j. \gamma_jk is the species
effect for species k on \beta_j.  \delta_jl is the city effect for city l on \beta_j.
\alpha_j is the shape parameter for \beta_j.


for each i.  Species random effect:
\[
\gamma_{ij} \sim N(0, \sigma_{\gamma i})
\]

species nested within Genus.  Genus nested within functional group.


for each j. City random effect:
\[
\delta_{jk} \sim N(0, \sigma_{\delta j})
\]


for j = 1
\[
\mu_1 = \tau_0 + \tau_1 * precip + \tau_2 * gdd
\]


I think that I should make the priors on the betas normal?  would that
allow for better correlation modelling?
...
#+begin_src R :exports results :results graphics :file ../figs/genus_none_species_single_cities_single_climate_none_hetero_no_family_gamma_prior_marginaleffects.png
marginal_effects(prior_mod, spaghetti = T, nsamples = 500, theme = theme_gray())
#+end_src

#+RESULTS:
[[file:../figs/genus_none_species_single_cities_single_climate_none_hetero_no_family_gamma_prior_marginaleffects.png]]

#+begin_src R :exports results :results graphics :file ../figs/genus_none_species_single_cities_single_climate_none_hetero_no_family_gamma_prior_pred.png
pp <- posterior_predict(prior_mod, nsamples=10)
ppc_dens_overlay(y = d$DBH, yrep=pp) + ylim(c(0,0.018))
#+end_src

#+RESULTS:
[[file:../figs/genus_none_species_single_cities_single_climate_none_hetero_no_family_gamma_prior_pred_.png]]
