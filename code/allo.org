...
Gradient evaluation took 0.001069 seconds
1000 transitions using 10 leapfrog steps per transition would take 10.69 seconds.
Adjust your expectations accordingly!
...
Iteration: 2000 / 2000 [100%]  (Sampling)
 ...
 Elapsed Time: 24.3724 seconds (Warm-up)
               19.1688 seconds (Sampling)
               43.5411 seconds (Total)

Iteration: 2000 / 2000 [100%]  (Sampling)

 Elapsed Time: 24.5135 seconds (Warm-up)
               20.9414 seconds (Sampling)
               45.4549 seconds (Total)
#+end_example
...
#+begin_src R
m_fixed <- stan("singleSpeciesLocation_hetero.stan", standata(m))
summary(m_fixed)
#+end_src
...
:   path[1]="singleSpeciesLocation_hetero.stan": No such file or directory
: 2: In file(fname, "rt") :
:   cannot open file 'singleSpeciesLocation_hetero.stan': No such file or directory
: Error in get_model_strcode(file, model_code) :
:   cannot open model file "singleSpeciesLocation_hetero.stan"
: Error in summary(m_fixed) : object 'm_fixed' not found
...
#+begin_src R :exports results :results graphics :file ../figs/marginal_1.png
cond <- data.frame("1" = 1)
  me <-   marginal_effects(m, points = T, condistions = cond)
