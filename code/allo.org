    ...
    f <- function(x,m = 255) {round(m * (x - min(x, na.rm = T)) / max(x, na.rm = T), 0) }
      ...
      d <- left_join(dl, pl) %>%
        ...
        filter(complete.cases(.)) %>%
          ...
          mutate(gdd_col = ifelse(gdd > 8750, NA, gdd),
                 gdd_col = ifelse(gdd < 1250, NA, gdd_col),
          ...
          rowwise() %>%
          mutate(col = ifelse(!is.na(red) & !is.na(blue) & !is.na(green), rgb(red, green, blue, maxColorValue = 255), NA))
...
#+begin_src R :exports results :results graphics :file ../figs/gdd_cont.png :width 600 :height 400
