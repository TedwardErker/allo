...
#+begin_src R :exports results :results graphics :file ../figs/GenusSpecies_estimates.png

  ggplot() +
      geom_line(data = me$AGE, aes(x = AGE, y = estimate__, color = Genus, group = cond__)) +
      geom_point(data = d, aes(x = AGE, y = DBH, color = Genus), inherit.aes = F, alpha = .5) +
      facet_wrap(~Genus)
