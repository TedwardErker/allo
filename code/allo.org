...
#+begin_src R :exports results :results graphics :file ../figs/CityScientificNameData_CityCentric_n_commonSpecies.png
     library(dplyr)
     library(ggplot2)
   ...
   d3 <- filter(d2, !is.na(ScientificName))
                ...
                ggplot(d3, aes(y = ScientificName, x = City, fill = n)) +
                          geom_raster() +
                          coord_equal() +
                          scale_fill_continuous(label = "number\n of trees\n sampled", breaks = seq(min(d2$n), max(d2$n), length.out = 3), type = "viridis") +
                          theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
