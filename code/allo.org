...
#+begin_src R :exports results :results graphics :file ../figs/weibull_tighter.png
    data <- readRDS("../data/tidy_age_dbh.rds") %>%
        select(DBH, AGE) %>%
        rename(x = AGE, y = DBH)
...
#    b <- 1:3
    ...
    #w <- function(a,b,c,x) a * (1 - exp(10^(-b) * x^c))
...
#+begin_src R
    ...
    data_form <- formula(DBH ~ b0 + 100 * b1 * (1 - exp(-(b2/100) * AGE^(b3))))
    b0_form <- formula(b0 ~ (1 | City) + (1 | Genus/Species))
    b1_form <- formula(b3 ~ Precip * GDD +  (1 | City) + (1 | Genus / Species))
    b2_form <- formula(b2 ~ (1 | City) + (1 | Genus/Species))
    b3_form <- formula(b1 ~ (1 | City) + (1 | Genus/Species))
