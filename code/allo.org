...
rate starts slow, increases

slow then increase then asympote.  Had to add intercept since 0 age is
actually when the trees were transplanted from the nursery to the
urban setting.  A modified weibull curve has worked well in forestry
and is the one we use here.  However, there are many other curves that
meet these criteria and could be used.
