...
#+begin_src R
  ...
  nlprior <- c(prior(normal(60, 20), nlpar = "b0"),
               prior(normal(0, 2), nlpar = "b1"),
               prior(normal(1, 2), nlpar = "b2"))

          m <- brm(form, chains = 2, cores = 2, data = d, prior = nlprior)
#+end_src
...
b0_Intercept    94.53      8.71    80.53   115.21        572 1.00
b1_Intercept     0.01      0.00     0.01     0.01        652 1.00
b2_Intercept     1.04      0.07     0.90     1.20        519 1.00

Family Specific Parameters:
      Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
sigma     5.79      0.41     5.06     6.69        612 1.00
...
#+begin_src R
d <- readRDS("../data/tidy_age_dbh.rds")
fram_Minne <- filter(d, SpCode == "FRAM", City == "Minneapolis, MN")
#+end_src

#+RESULTS:

#+begin_src R :exports results :results graphics :file ../figs/fram_minne_age_dbh.png
ggplot(fram_Minne, aes(x = AGE, y = DBH)) + geom_point()
#+end_src

#+RESULTS:
[[file:../figs/fram_minne_age_dbh.png]]

I divide b0 by 100 to get the parameters on the same scale ish, with
...
#+begin_src R
  ...
  model_fram_FtCollins_homo <- brm(form, chains = 2, cores = 2, data = d, prior = nlprior)
#+end_src
