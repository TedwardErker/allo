...
#+begin_src R :exports results :results graphics :file ../figs/GenusSpecies_wpreds.png :width 800
#    pred <- predict(mod, newdata = d)
 # pred.df <- cbind(d, pred)
  ...
  ggplot(d, aes(x = AGE, y = DBH, color = Species)) +
          geom_point(alpha = .5) +
          facet_wrap(~Genus) +
        geom_line(data = pred.df, aes(x = AGE, y = Estimate, group = Species), inherit.aes = F) +
        geom_ribbon(data = pred.df, aes(x = AGE, ymin = Q2.5, ymax = Q97.5, color = Species), size = .1,inherit.aes = F, fill = NA)
#+end_src
...
#    pred <- predict(mod, newdata = d)
 # pred.df <- cbind(d, pred)
  ...
  ggplot(d, aes(x = AGE, y = DBH, color = Species)) +
          ...
          facet_wrap(~Genus) +
        geom_line(data = pred.df, aes(x = AGE, y = Estimate, group = Species), inherit.aes = F) +
        geom_ribbon(data = pred.df, aes(x = AGE, ymin = Q2.5, ymax = Q97.5, fill = Species), inherit.aes = F, alpha = .2)
#+end_src
