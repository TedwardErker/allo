...
for each beta, j = 0,2,3, species, k, and each city, l.

\[
\beta_{jkl} = \beta_j + \gamma_{jk}+\delta_{jl}
\]

for beta_1
\[
\beta_{1kl[i]} = \beta_1 + \tau_1 * Precip_i + \tau_2 * GDD_i + \tau_3 *
(Precip_i * GDD_i) + \gamma_{1k[i]}+\delta_{1l[i]}
\]

where \beta_j is the mean for \beta coefficient \beta_j. \gamma_jk is
the contribution of genetic (species) effect for species k on \beta_j.
\delta_jl is the city effect for city l on \beta_j.
