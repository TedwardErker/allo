...
The order wasn't quite fixed!!!!

  me2 <- me$AGE  %>% mutate(City = factor(City, levels=unique(City[order(GDD*Precip)]), ordered=TRUE))

      ggplot(me2) +
          geom_line(aes(x = AGE, y = estimate__, color = Precip * GDD), size = 2) +
          geom_ribbon(aes(x = AGE, ymin = estimate__ - se__, ymax = estimate__ + se__), alpha = .3, fill = "blue") +
          geom_point(data = filter(mod$data, Species == "List"), aes(x = AGE, y = DBH), alpha = .3, size = .5) +
