...
Indianapolis is missing too.  There were some irregularities measuring
the age in these cities (see the report).
...
#+BEGIN_SRC sh :session a :results verbatim
#rsync -avz erker@krusty:~/allo/models/genus_many_species_many_cities_many_climate_b3linint_hetero_no_family_Gammatighter_priors.rds ~/git/allo/models/
rsync -avz erker@krusty:~/allo/models/genus_many_species_many_cities_many_climate_b3linint_hetero_no_family_Gammalooser_priors.rds ~/git/allo/models/
#+END_SRC
...
: erker@krusty's password:
: receiving file list ... done
: genus_many_species_many_cities_many_climate_b3linint_hetero_no_family_Gammalooser_priors.rds
:
: sent 38 bytes  received 24011107 bytes  3201486.00 bytes/sec
: total size is 24195320  speedup is 1.01
...
#+begin_src R :exports results :results graphics :file ../figs/predictions_dbh_bySpeciesCity_wData.png :height 1200 :width 1200
  ...
  predictions_apprange_subspecies <- filter(predictions_apprange, Species %in% to.include)

  blue <- "blue"

  ##   ggplot() +
  ##       geom_point(data = d, aes( x = AGE, y = DBH), alpha = .7, size = 1) +
  ##       facet_grid(Region~Species) +
  ##       theme_minimal() +
  ##       theme(text = element_text(size = 20),
  ##             legend.position = "none") +
  ##       geom_line(data = predictions_apprange_subspecies, aes(x = AGE, y = predicted_dbh), size = 1.5, color = blue)
  ...
  dd <- left_join(dd, unique(select(d, Region, City)))
      ...
      ggplot(me2) +
          ...
          geom_ribbon(aes(x = AGE, ymin = estimate__ - se__, ymax = estimate__ + se__), alpha = .3, fill = "blue") +
          geom_point(data = dd, aes(x = AGE, y = DBH), alpha = 1, size = .5, color = "red") +
          facet_grid(Species~Region*gddprecip) +
          geom_line(data = predictions_apprange_subspecies, aes(x = AGE, y = predicted_dbh), size = 1.5, color = blue)

#+end_src

#+RESULTS:
[[file:../figs/predictions_dbh_bySpeciesCity_wData.png]]
