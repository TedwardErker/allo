...
#+begin_src R
    ...
    model_table <- read.csv("../data/model_table.csv", stringsAsFactors = F)
      ...
      generate_formula <- function(model_table, gen, sp, cit, clim, het) {
          filter(model_table, genus == gen, species == sp, cities == cit, climate == clim, hetero == het) %>%
              mutate(form = ifelse(sigma == "", paste0("bf(", data,", ",b0,", ",b1,", ",b2, ", nl = T)"),  paste0("bf(", data,",",b0,",",b1,",",b2,",",sigma, ", nl = T)"))) %>%
              pull(form)
      }
...
The following object is masked from ‘package:dplyr’:
    ...
    combine
[1] "bf(y ~ 100*b0 * (1 - exp(-(b1/100) * x ^ b2)), b0 ~ 1, b1 ~ 1, b2 ~ 1, nl = T)"
...
#+begin_src R
  ...
  m <- brm(form, chains = 2, cores = 2, data = dt, prior = nlprior, iter = 500)
#+end_src
