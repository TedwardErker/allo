...
all sigmas:
for beta_0, the intercept.  realistic values for this range from 1 to
maybe 10 (that would be a very large average size to plant).
Therefore, the sd for \delta, \sigma_{0 c[i]}, is at most 3, I think
it's most likely less than 2.  I'll set the prior so that there's a
95% chance it's less than 2.  Variation between genus/species is
probably similar.

\[
\sigma_{0_c[i]} \sim \text{Normal}(0, .67)
\]

\[
\sigma_{0_s[i]} \sim \text{Normal}(0, .67)
\]

\[
\sigma_{0_g[i]} \sim \text{Normal}(0, .67)
\]
