...
#+begin_src R
    ...
    x2 <- colorRampPalette(c("blue","green","yellow"), space = "rgb", interpolate = "spline")(100)
    x3 <- colorRampPalette(c("#5502ed","#41afab","#25db00"), space = "rgb", interpolate = "spline")(100)

    cols <- sapply(1:length(x1), function(i) {colorRampPalette(c(x1[i], x2[i], x3[i]), space = "rgb",interpolate = "spline")(100) })
...
#+begin_src R :exports results :results graphics :file ../figs/hue_pal.png
  ggplot(data = cols, aes( x = gdd, y = precip, fill = value)) +
    geom_raster() +
    scale_fill_identity()
#+end_src
