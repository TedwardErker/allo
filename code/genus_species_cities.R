...
library(dplyr)
  ...
  nlprior <- c(prior(gamma(9, 3), nlpar = "b0",lb = 0),
               ...
               prior(normal(0, .2), class = "sd", nlpar = "b0", group = "City"),
#               prior(normal(0, .1), class = "sd", nlpar = "b1", group = "City"),
  ...
  mod <- brm(form,
             ...
             prior = nlprior,
             family = Gamma("identity"),
             chains = 6, cores = 6, init_r = .1, iter = 500, control = list(adapt_delta = .8))
