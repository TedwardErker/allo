...
nlprior <- c(prior(gamma(9, 3), nlpar = "b0",lb = 0),
             ...
             prior(normal(0, .3), class = "sd", nlpar = "b1", group = "City"),
             ...
             prior(normal(0, .4), class = "sd", nlpar = "b1", group = "Genus"),
...
mod <- brm(form,
           ...
           prior = nlprior,
           family = Gamma("identity"),
           chains = 12, cores = 12, init_r = .3, iter = 500)

saveRDS(mod, paste0("../models/genus_",genus,"_species_",species,"_cities_", cities, "_climate_", climate, "_hetero_", hetero, "_family_", family, "looser_priors.rds"))
